...
const binRequests15MinutesOver8Hours = (requests) => {
  ...
  const handleSendNewRequest = (event) => {
    ...
    settings.extra.forEach(property => {
      if(property.required) {
        var reqOption = newRequestOptions.find(o => o._id === property._id)
        if(!reqOption) { //required property value not provided
          scanMissingRequiredOptions.push(property._id);
        } else if(property.type === Properties.PHONE_NUM_PROPERTY && reqOption.value && reqOption.value.trim().length !== 13) {//special validation for phone number
          scanMissingRequiredOptions.push(property._id);
        }
      }
    });
  ...
  const translateOptionValue = (forOption) => {
    var property = settings.extra.find(o => o._id === forOption._id);
    if(property) {
              ...
              <TableBody>
                ...
                :last8hoursRequests.map((request) => (
                  <TableRow key={request._id}>
                    <TableCell style={{fontSize:12}}>{request.requestFor.label}</TableCell>
                    ...
                    >
                      {request.options.map(r => (
                        <Grid item key={r._id}>
                          ...
                          {r.label} : {translateOptionValue(r)} 
                          </Typography>
                        </Grid>
                      ))}
                    </Grid>
