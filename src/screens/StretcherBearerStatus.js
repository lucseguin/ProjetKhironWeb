  ...
  useEffect(() => {
    ...
    .then(
      axios.spread((...responses) => {
        ...
        if (globalSettingsRes.status === 200 && globalSettingsRes.data.length > 0) {
  ...
  const binRequests15MinutesOver8Hours = (requests) => {
    ...
    if(requests && requests.length > 0) {
      ...
      requests.forEach((request) => {
        ...
        var bucketKeyT = addZero(requestedOn.getHours()) + ":" + addZero(Math.floor(requestedOn.getMinutes()/15)*15);
        var dataPointT = map.get(bucketKeyT);
        if(!dataPointT)
          dataPointT = { name:bucketKeyT, requested: 0, completed:0, averageTime: 0 };

        dataPointT.requested = dataPointT.requested + 1;
        if(request.completedOn!=='1970-01-01T00:00:00.000Z') {
          dataPointT.completed = dataPointT.completed + 1;  
          var minutes = Math.floor((new Date(request.completedOn).getTime() - requestedOn.getTime())/60000);
          dataPointT.averageTime = dataPointT.averageTime + minutes; //in minutes 
        }

        map.set(bucketKeyT,dataPointT);
      });
        ...
        <Fade in={openNewRequest}>
          ...
          <Grid container
            ...
            <Grid container
              ...
              <Grid item style={{ maxHeight: 500, overflowY: 'auto', overflowX: 'hidden'}}>
                <Paper elevation={3} >
                <Grid container
                direction="column"
                justify="flex-start"
              ...
              <TableBody>
                ...
                :last8hoursRequests.map((request) => (
                  <TableRow key={request._id}>
                    <TableCell style={{width:30, fontSize:12}}>
                    ...
                    <TableCell style={{fontSize:12}}>{request.from.label}</TableCell>
                    <TableCell style={{fontSize:12}}>{request.to.label}</TableCell>
                    <TableCell style={{fontSize:12}}>{request.assigned?request.assigned.label:null}</TableCell>
                    <TableCell style={{fontSize:12}}>{new Date(request.requestedOn).toLocaleString('fr-CA', {dateStyle:"short", timeStyle:"short", hour12:false})}</TableCell>                   
                    <TableCell style={{fontSize:12}}>{request.assignedOn!=='1970-01-01T00:00:00.000Z'?new Date(request.assignedOn).toLocaleString('fr-CA', {dateStyle:"short", timeStyle:"short", hour12:false}):null}</TableCell>
                    <TableCell style={{fontSize:12}}>{request.completedOn!=='1970-01-01T00:00:00.000Z'?new Date(request.completedOn).toLocaleString('fr-CA', {dateStyle:"short", timeStyle:"short", hour12:false}):null}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
