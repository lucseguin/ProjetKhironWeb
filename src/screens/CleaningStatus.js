...
/*   const [loadingToBeds, setLoadingToBeds] = useState(false);
  ...
  const loadRequestDataEx = (sLevel) => {
    ...
    let last30Days =  axios.get("/projetkhiron/cleaner/requests/stats", {
      params: {
          type: '30-days'
        }
    });

    let last7Days =  axios.get("/projetkhiron/cleaner/requests/stats", {
      params: {
          type: '7-days'
        }
    });

    let last8hours =  axios.get("/projetkhiron/cleaner/requests", {
      params: {
          from: moment().subtract(8, 'hours').toDate()
        }
    });

    let bearerAnalysis =  axios.get("/projetkhiron/cleaner/analysis", {
      params: {
        type:'outOfService',
        seviceLevel: sLevel
        }
  ...
  return (
    <div className={classes.root} >
            ...
            <Grid container
              ...
              <Grid xs item>
                <Typography variant="h6">
                  Demande de Nettoyage et salubrité
                </Typography>
              </Grid>
              <Grid xs item>
                <Autocomplete
              ...
              <Grid item xs >
                  ...
                  <Button variant="contained" color="primary" disabled={!selectedFromRequest} startIcon={<AddCircleOutlineIcon />} onClick={handleSendNewRequest}>
                      Demander
                  </Button>
              </Grid> 
            </Grid> 
      ...
      <Grid container
        ...
        <Grid item>
          <Typography variant="h6">
            Demande de nettoyage et salubrité derniers 8 heures
          </Typography>
        </Grid>
        <Grid item>
